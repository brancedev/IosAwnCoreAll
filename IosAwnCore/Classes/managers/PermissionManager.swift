...
typealias ActivityCompletionHandler = () -> Void
...
public class PermissionManager {
    ...
    public func areNotificationsGloballyAllowed(
        whenGotResults permissionCompletion: @escaping (Bool) -> Void
    ) {
        ...
        current.getNotificationSettings(completionHandler: { (settings) in
            ...
            if settings.authorizationStatus == .notDetermined {
                ...
                if settings.authorizationStatus == .provisional {
                    // Provisional permission was granted (iOS 12.0+)
                    permissionCompletion(true)
                    return
                }
            }
    ...
    public func shouldShowRationale(
        ...
        whenGotResults completion: @escaping ([String]) -> Void
    ) {
        var shouldShowRationaleList: [String] = []

        self.areNotificationsGloballyAllowed(
whenGotResults: { (areAllowed) in
...
completionHandler: {
                    ...
                    for permission in permissions {
                        if let permissionEnum: NotificationPermission = NotificationPermission.fromString(
                            permission)
                        {
                            ...
                            case .Car:
                                if iOSpermissions.carPlaySetting != .enabled {
                                    shouldShowRationaleList
                                        .append(
                                            NotificationPermission.Car.rawValue
                                        )
                                }
                                break
                            ...
                            case .Provisional:
                                if #available(iOS 12.0, *) {
                                    if iOSpermissions.authorizationStatus != .provisional {
                                        shouldShowRationaleList
                                            .append(
                                                NotificationPermission.Provisional.rawValue
                                            )
                                    }
                                }
                                break
...
completionHandler: {
                    ...
                    for permission in permissions {
                            ...
                            permission)
                        {
                            ...
                            case .Alert:
                                if iOSpermissions.alertSetting == .enabled {
                                    if channelKey == nil
                                        || self.isSpecifiedChannelPermissionAllowed(
                                            channelKey: channelKey!, permissionEnum: permissionEnum)
                                    {
                                        allowed
                                            .append(
                                                NotificationPermission.Alert.rawValue
    ...
    public func isSpecifiedPermissionGloballyAllowed(
        _ permission: String,
        channel: String?,
        completion: @escaping (
            Bool
...
completionHandler: {
 ...
 .CriticalAlert:
                    if #available(iOS 12.0, *) {
                        completion(
                            iOSpermissions.criticalAlertSetting != .disabled
                        )
                    } else {
                        completion(true)
                    }
                ...
                case .Provisional:
                    if #available(iOS 12.0, *) {
                        completion(
                            iOSpermissions.authorizationStatus == .provisional
                        )
                    } else {
                        completion(true)
                    }
    ...
    public func isSpecifiedChannelPermissionAllowed(
        channelKey: String, permissionEnum: NotificationPermission
    ) -> Bool {
    ...
    public func requestUserPermissions(
        _ permissions: [String],
        filteringByChannelKey channelKey: String?,
        whenUserReturns permissionCompletion: @escaping ([String]) -> Void
    ) throws {
        ...
        arePermissionsAllowed(
            ...
            whenGotResults: { (permissionsAllowed) in
                ...
                let permissionsNeeded: [String] = permissions.filter(
                    { !permissionsAllowed.contains($0)
                    })
                ...
                if permissionsNeeded.isEmpty {
                    ...
                    UNUserNotificationCenter
                        ...
                        .getNotificationSettings { (settings) in
                            ...
                            if #available(iOS 12.0, *) {
                                if permissionsRequested
                                    ...
                                    || permissionsRequested.contains(
                                        NotificationPermission.OverrideDnD.rawValue
                                    )
                                {
                            ...
                            self.shouldShowRationale(
                                permissionsRequested,
 filteringByChannelKey: channelKey,
                                whenGotResults: { listToShowRationale in
                                    ...
                                    if listToShowRationale.count == 1 {
                                        ...
                                        guard
                                            let permissionEnum = NotificationPermission.fromString(
    ...
    private func showRequestDialog(
        ...
        permissionCompletion: @escaping ([String]) -> Void
    ) {

        let iOSpermissions: UNAuthorizationOptions = getIosPermissionsCode(
            permissionsToRequest
        )
        UNUserNotificationCenter
            .current()
            .requestAuthorization(options: iOSpermissions) {
                (granted, error) in
    ...
    private func refreshReturnedPermissions(
        _ permissions: [String],
        filteringByChannelKey channelKey: String?,
        whenUserReturns completionHanlder: @escaping ([String]) -> Void
    ) {
        arePermissionsAllowed(
            permissions,
            ...
            whenGotResults: { (permissionsAllowed: [String]) in
                ...
                completionHanlder(
                    permissions.filter { !permissionsAllowed.contains($0)
                    })
            })
    }

    public func updateChannelModelThroughPermissions(
        channelKey: String,
        permissions: [String]
    ) {
    ...
    public func showNotificationConfigPage(
        whenUserReturns completionHandler: @escaping () -> Void
    ) {
